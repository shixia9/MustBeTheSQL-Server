...
public class MysqlMetaData implements MetaData {
    ...
    public String tableDDL(Connection connection, String schemaName, String tableName) {
        String targetSchema = schemaName != null ? schemaName : "";
        String sql = "SHOW CREATE TABLE `" + (targetSchema.isEmpty() ? "" : targetSchema + "`.`") + tableName + "`";
        try (PreparedStatement stmt = connection.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
            ...
            // It might be a view
            sql = "SHOW CREATE VIEW `" + (targetSchema.isEmpty() ? "" : targetSchema + "`.`") + tableName + "`";
            try (PreparedStatement stmt = connection.prepareStatement(sql);
                 ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return rs.getString(2);
